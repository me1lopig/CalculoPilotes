...
with tab_datos:
    ...
    # --- VALIDACIÓN ESTRICTA DE ESPESORES Y VALORES NEGATIVOS ---
    ...
    if valores_negativos:
        st.error(
            "⛔ **Error de datos:** Se han detectado valores negativos en las propiedades "
            "del terreno (Gamma, cohesión o ángulo de rozamiento). Estos parámetros deben ser ≥ 0."
        )
        st.session_state.calculado = False

    # Variable unificada para saber si hay algún error que deba bloquear el cálculo
...
st.sidebar.markdown("---")
# BOTÓN BLOQUEADO SI HAY ERRORES EN LA TABLA (Espesores o Valores Negativos)
...
if st.session_state.calculado:
    ...
    if not df_res.empty:
        ...
        with tab_matriz_punta:
            ...
            st.markdown("---")
            st.subheader("📈 Curvas de Capacidad Admisible por Punta")
            ...
            fig_punta = px.line(df_plot_punta, x="L", y="Q_punta_adm (kN)", color="Diámetro", markers=True,
                                title=f"Capacidad por Punta vs. Longitud (FS = {FS:.2f})",
                                color_discrete_sequence=px.colors.qualitative.Set1,
                                template="plotly_white")
            
            fig_punta.update_layout(xaxis_title="Longitud L (m)", yaxis_title="Carga por Punta (kN)", hovermode="x unified")
            fig_punta.update_traces(line=dict(width=3), marker=dict(size=8))
        ...
        with tab_matriz_fuste:
            ...
            st.markdown("---")
            st.subheader("📈 Curvas de Capacidad Admisible por Fuste")
            ...
            fig_fuste = px.line(df_plot_fuste, x="L", y="Q_fuste_adm (kN)", color="Diámetro", markers=True,
                                title=f"Capacidad por Fuste vs. Longitud (FS = {FS:.2f})",
                                color_discrete_sequence=px.colors.qualitative.Set1,
                                template="plotly_white")
            
            fig_fuste.update_layout(xaxis_title="Longitud L (m)", yaxis_title="Carga por Fuste (kN)", hovermode="x unified")
            fig_fuste.update_traces(line=dict(width=3), marker=dict(size=8))
        ...
        with tab_formulacion:
            ...
            st.markdown(r"$f_D = 1 - \frac{D}{3} \ge \frac{2}{3}$")
            
            st.markdown("**1.2. Punta a Corto Plazo (Condición No Drenada)**")
            st.markdown(r"$q_p = 9 \cdot c_u \cdot f_D$")
            
            st.markdown("**1.3. Punta a Largo Plazo (Condición Drenada)**")
            st.markdown(r"$q_p = \sigma'_{v,p} \cdot N_q^* + c' \cdot N_c^*$")
            st.markdown("Donde los factores de capacidad de carga, afectados por la escala, son:")
            st.markdown(r"$N_q^* = 1.5 \cdot \frac{1 + \sin(\phi)}{1 - \sin(\phi)} \cdot e^{\pi \cdot \tan(\phi)} \cdot f_D$")
            st.markdown(r"$N_c^* = \frac{N_q^* - 1}{\tan(\phi)}$")
            ...
            st.markdown(r"$\tau_f = c_u \cdot \frac{100}{100 + c_u} \le 70 \text{ kPa}$")
            
            st.markdown("**2.2. Fuste a Largo Plazo (Ley Mohr-Coulomb)**")
            st.markdown(r"$\tau_f = c' + \sigma'_v \cdot K_0 \cdot \tan(\delta) \le 90 \text{ kPa}$")
            ...
            st.markdown(r"$Q_{tope} = A \cdot \sigma_{tope} \cdot 1000$")
