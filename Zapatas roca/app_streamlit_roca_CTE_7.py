...
# --- Fallback Matplotlib -> PNG (no requiere Chromium) ---
def plot_png_fallback_matplotlib(b_vals, qd_vals, titulo="Curva de Presión Admisible"):
    fig, ax = plt.subplots(figsize=(10, 6), dpi=150)
    ax.plot(b_vals, qd_vals, linewidth=3)
    ax.set_title(titulo)
    ax.set_xlabel("Ancho B (m)")
    ax.set_ylabel("qd (MPa)")
...
def generar_informe_word(qu_inp, s_val, a_val, estado_j, b_range_info, checks, df_data, fig_plot, b_smooth, qd_smooth):
    doc = Document()
    ...
    p_params.add_run(f"• Anchos (B): {b_range_info[0]:.2f}m - {b_range_info[1]:.2f}m (paso {b_range_info[2]:.2f}m)")
    ...
    try:
        # Intento 1: Plotly -> PNG (requiere kaleido funcionando)
        img_bytes = fig_plot.to_image(format="png", width=1000, height=600, scale=3)
    except Exception:
        # Fallback: Matplotlib (sin Chromium)
        img_bytes = plot_png_fallback_matplotlib(b_smooth, qd_smooth)
        ...
        with st.spinner("Generando informe..."):
            try:
                buffer = generar_informe_word(
                    qu_calc, s, a, estado_junta, [b_min, b_max, b_step],
